...
import { ReviewDataSearchParams, isReviewDataSearchParams, reviewCompressor, getBeerWithIdOrThrow } from '@/utils/bingo-helpers';
import { matchesSearchQuery } from '@/utils/misc';
import { ArrowDownIcon, ArrowUpIcon } from '@chakra-ui/icons';
import { Box, Button, Divider, IconButton, Input, InputGroup, InputRightAddon, Select, Stack } from '@chakra-ui/react';
  ...
  const visibleReviews = useMemo(
    () =>
      reviews.filter((review) => {
        const beer = getBeerWithIdOrThrow(review.beerId);
    ...
    <>
      <Box>
        <Stack gap="2">
          ...
          <InputGroup>
            <Select roundedRight="none" value={sortField} onChange={(e) => setSortField(e.target.value as SortableField)}>
            ...
            <InputRightAddon>
              <IconButton
                ...
                icon={sortDirection === 'asc' ? <ArrowDownIcon /> : <ArrowUpIcon />}
        ...
        <Divider my="4" />
        <ReviewList sortField={sortField} sortDirection={sortDirection} reviews={reviews} search={deferredSearch} />
      </Box>
      <ReviewShareModal />
    </>
  );
